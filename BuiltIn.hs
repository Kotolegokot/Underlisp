...
builtin_str_length :: Eval -> Context -> [SExpr] -> IO SExpr
builtin_str_length eval context [arg] = do
  expr <- eval context arg
  case expr of
    SString string -> return . SInt . length $ string
    _     -> error "string expected"

builtin_str_length _    _       _     = error "str-length requires only one argument"

builtin_head :: Eval -> Context -> [SExpr] -> IO SExpr
...
builtin_tail :: Eval -> Context -> [SExpr] -> IO SExpr
...
builtin_init :: Eval -> Context -> [SExpr] -> IO SExpr
...
builtin_last :: Eval -> Context -> [SExpr] -> IO SExpr
...
builtin_length :: Eval -> Context -> [SExpr] -> IO SExpr
...
builtin_append :: Eval -> Context -> [SExpr] -> IO SExpr
...
builtin_nth :: Eval -> Context -> [SExpr] -> IO SExpr
